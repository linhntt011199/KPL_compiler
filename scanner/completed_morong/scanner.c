...
void skipComment() {
  ...
  while (1) {
    ...
    } else if (charCodes[currentChar] == CHAR_TIMES) { // Next is *
      readChar(); // Get next character

      if (currentChar == EOF) { // End of File
	error(ERR_ENDOFCOMMENT, lineNo, colNo); // Comment end with EOF
      } else if (charCodes[currentChar] == CHAR_RPAR) { // Next is )
	  // End of comment
	  readChar();
	  return;
 	}
...
Token* readIdentKeyword(void) {
  ...
  do {
    ...
    // Get next character
    readChar();
  } while (currentChar != EOF && count < MAX_IDENT_LEN && (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT));
  // End string
